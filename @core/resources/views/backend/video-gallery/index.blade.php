...
<th>{{__('Image')}}</th>
                                            ...
                                            @foreach($all_gallery_images as $data)
                                                <tr>
                                                    ...
                                                    <td><a class="text-white" href="{{route('frontend.gallery.video.single',$data->id)}}">{{$data->id}}</a></td>
                                                    ...
                                                    <td>
                                                        <a tabindex="0" class="btn btn-danger btn-xs mb-3 mr-1"
                                                           ...
                                                           <form method='post' action='{{route('admin.gallery.video.delete',$data->id)}}'>
...
{{--                            @endforeach--}}
            ...
            <div class="col-lg-5 mt-5">
                <div class="card">
                    <div class="card-body">
                        <h4 class="header-title">{{__('Add New Video')}}</h4>
                        <form action="{{route('admin.gallery.video.new')}}" method="post" enctype="multipart/form-data">
                            @csrf
    ...
    <div class="modal fade" id="testimonial_item_edit_modal" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form action="{{route('admin.gallery.video.update')}}" id="testimonial_edit_modal_form"  method="post" enctype="multipart/form-data">
        ...
        $(document).ready(function () {
            ...
            $(document).on('click','#bulk_delete_btn',function (e) {
                ...
                if(allIds != '' && bulkOption == 'delete'){
                    ...
                    $.ajax({
                        'type' : "POST",
                        'url' : "{{route('admin.gallery.video.bulk.action')}}",
                        'data' : {
                            _token: "{{csrf_token()}}",
                            ids: allIds
                        },
            ...
            $(document).on('change','select[name="lang"]',function (e) {
                ...
                $.ajax({
                    url : "{{route('admin.gallery.video.category.by.lang')}}",
                    type: "POST",
                    data: {
                        _token : "{{csrf_token()}}",
                        lang: selectedLang
