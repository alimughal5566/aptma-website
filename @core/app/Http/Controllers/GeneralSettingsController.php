...
class GeneralSettingsController extends Controller
{
    ...
    public function update_smtp_settings(Request $request)
    {
        ...
        $env_val['MAIL_MAILER'] = !empty($request->site_smtp_mail_mailer) ? $request->site_smtp_mail_mailer : 'MAIL_MAILER';
    ...
    public function update_typography_settings(Request $request)
    {
        ...
        $body_font_variant = !empty($request->body_font_variant) ?  $request->body_font_variant: ['400'];
        $heading_font_variant = !empty($request->heading_font_variant) ?  $request->heading_font_variant: ['400'];
    ...
    public function update_page_settings(Request $request)
    {
        ...
        foreach ($slug_list as $slug_field){
            update_static_option($slug_field, Str::slug($request->$slug_field));
        }
    ...
    public function update_payment_settings(Request $request)
    {
        ...
        setEnvValue([
            ...
            'RAVE_ENVIRONMENT' => getenv('APP_ENV') == 'local' ? "staging" : "live"
        ]);
